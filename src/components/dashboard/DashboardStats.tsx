  ...
  return (
    <div className="space-y-8 animate-page-transition max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
           <h1 className="text-3xl font-extrabold tracking-tight leading-none text-slate-900 uppercase">Tổng quan <span className="text-gradient-primary">V1</span></h1>
           <p className="text-[10px] font-bold text-slate-400 mt-2.5 uppercase tracking-[0.3em] ml-1">Live tracking system</p>
        </div>
        <div className="flex items-center gap-3 bg-white/80 backdrop-blur-md px-5 py-3 rounded-2xl border border-white/60 shadow-sm">
           <div className="glowing-dot-active"></div>
           <span className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">Hệ thống đang trực tuyến</span>
        </div>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Kho thiết bị', val: stats.totalToolsQuantity, icon: Package, color: 'indigo', highlight: 'indigo' as const },
          { label: 'Yêu cầu mới', val: stats.pendingRequests, icon: TrendingUp, color: 'purple', highlight: 'violet' as const },
          { label: 'Sẵn sàng', val: stats.totalToolsAvailable, icon: Wrench, color: 'emerald', highlight: 'emerald' as const },
          { label: 'Đang mượn', val: stats.borrowedTools, icon: ShoppingBag, color: 'amber', highlight: 'amber' as const }
        ].map((c, i) => (
          <Card key={i} highlightColor={c.highlight} className="hover-lift border-none group relative overflow-hidden bg-white/80 backdrop-blur-md glass-card-hover">
            ...
            <div className="flex justify-between items-start mb-6 relative">
              <div className={`p-4 bg-${c.color}-50 text-${c.color}-600 rounded-2xl border border-${c.color}-100/50 shadow-sm`}>
                <c.icon className="w-6 h-6" />
              </div>
              <Activity className="w-4 h-4 text-slate-300" />
            </div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{c.label}</p>
            <p className="text-3.5xl font-extrabold text-slate-900 tabular-nums tracking-tight">{c.val}</p>
          </Card>
        ))}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <Card className="lg:col-span-8 p-8 md:p-10 border-none shadow-xl bg-white" highlightColor="indigo" title="Hoạt động mượn đồ">
          <div className="mb-8">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">Thống kê 7 ngày gần nhất</p>
          </div>
          <SafeChartContainer height={300} hasData={barData.some(d => d.requests > 0)}>
            <BarChart data={barData} margin={{ top: 0, right: 0, left: -25, bottom: 0 }}>
              ...
              <Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 20px 40px rgba(0,0,0,0.05)', padding: '12px' }} />
              <Bar dataKey="requests" fill="url(#barGradient)" radius={[8, 8, 0, 0]} barSize={28} />
            </BarChart>
          </SafeChartContainer>
        </Card>

        <Card className="lg:col-span-4 p-8 md:p-10 border-none shadow-xl bg-white flex flex-col" highlightColor="violet" title="Tình trạng kho">
          <div className="flex-1 min-h-[200px] flex items-center justify-center mt-6">
            <SafeChartContainer height={220} hasData={pieData.length > 0}>
              <PieChart>
                ...
                <text x="50%" y="50%" textAnchor="middle" dominantBaseline="middle" className="text-slate-900 text-3xl font-extrabold tracking-tight">{stats.totalToolsQuantity}</text>
              </PieChart>
            </SafeChartContainer>
          </div>
          ...
          <div className="grid grid-cols-1 gap-2 mt-8">
              {pieData.map((entry, index) => (
                  <div key={index} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50/50 border border-slate-100 transition-colors hover:bg-slate-50">
