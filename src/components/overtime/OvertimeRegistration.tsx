...
export const OvertimeRegistration: React.FC<OvertimeRegistrationProps> = ({ user }) => {
  ...
  const updateDayWorkingStatus = async (status: boolean) => {
    ...
    try {
        ...
        alert(`Đã lưu thiết lập: Ngày ${selectedDateStr.split('-').reverse().join('/')} hiện là ${status ? 'NGÀY ĐI LÀM (OT sau ca)' : 'NGÀY NGHỈ (OT cả ngày)'}`);
    } catch (err: any) {
        console.error("Day config error:", err);
        alert("Lỗi cập nhật ngày: " + (err.message || "Không rõ nguyên nhân"));
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Refresh existing check with DB
      ...
      const payload = {
          ...
          reason: reason.trim() || 'Làm thêm ca', 
          status: 'approved' 
      };
      ...
      let res;
      if (finalEditingId) {
        res = await supabase.from('ot_requests').update(payload).eq('id', finalEditingId).select();
      } else {
        res = await supabase.from('ot_requests').insert([payload]).select();
      }
      ...
      console.log("[Overtime] Submit success:", res.data);
      ...
      await fetchRequests();
        ...
        alert("Lỗi: " + (err.message || "Không rõ nguyên nhân") + (err.code ? ` [Code: ${err.code}]` : ""));
    } finally {
    ...
    for (let d = 1; d <= totalDays; d++) {
        ...
        days.push(
            <div 
                ...
                className={`h-16 md:h-32 border border-slate-100 p-1 md:p-2 relative transition-all cursor-pointer group flex flex-col 
            ...
            >
                <div className="flex justify-between items-start">
                    <span className={`text-[9px] md:text-[11px] font-bold ${isToday ? 'bg-indigo-600 text-white w-3.5 h-3.5 md:w-5 md:h-5 rounded-md flex items-center justify-center shadow-sm' : 'text-slate-500'}`}>
                        {d}
                    </span>
                    {!isActuallyWorking && (
        ...
        {isModalOpen && (
            <div className="fixed inset-0 bg-slate-900/60 z-[100] flex items-end md:items-center justify-center backdrop-blur-md animate-fade-in px-0 md:px-6">
                <div className="bg-white rounded-t-[2.5rem] md:rounded-[3rem] shadow-2xl w-full max-w-2xl animate-zoom-in overflow-hidden max-h-[96vh] flex flex-col border border-white/20">
                    ...
                    <div className="p-6 md:p-10 space-y-8 overflow-y-auto no-scrollbar pb-20 bg-slate-50 flex-1 relative">
                        {message && (
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center justify-between ml-1">
                                <h4 className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400">Nhân sự OT ngày {selectedDateStr.split('-').reverse().join('/')}</h4>
                                {isAdmin && (
                                    <button 
                                      type="button"
                                      onClick={() => {
                                      ...
                                      className="text-[8px] font-bold text-indigo-500 uppercase px-2 py-1 bg-indigo-50 rounded hover:bg-indigo-100"
                                    >
                                      + Người mới
                                    </button>
                                )}
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-40 overflow-y-auto no-scrollbar p-1">
                        ...
                        {isAdmin && (
                            <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-5">
                                <div className="flex items-center gap-3">
                                    ...
                                    <h4 className="text-[10px] font-extrabold uppercase tracking-widest text-slate-900">Thiết lập ngày công (Admin)</h4>
                                </div>
                                <div className="grid grid-cols-2 gap-3 relative">
                                    <button 
                                        type="button"
                                        onClick={() => updateDayWorkingStatus(false)} 
                                        className={`py-4 rounded-xl border-4 transition-all font-black text-[10px] uppercase tracking-widest shadow-sm ${ (isSelectedDayWorking === false) ? 'bg-rose-600 text-white border-rose-300' : 'bg-white text-slate-400 border-slate-100 hover:border-rose-200' }`}
                                    >
                                        NGÀY NGHỈ
                                    </button>
                                    <button 
                                        ...
                                        className={`py-4 rounded-xl border-4 transition-all font-black text-[10px] uppercase tracking-widest shadow-sm ${ (isSelectedDayWorking === true) ? (isTet ? 'bg-red-600 border-red-300 text-white' : 'bg-indigo-600 border-indigo-300 text-white') : 'bg-white text-slate-400 border-slate-100 hover:border-indigo-200' }`}
                                    >
                                        NGÀY ĐI LÀM
                                    </button>
                                    {isConfigUpdating && (
                                ...
                                <p className="text-[9px] text-slate-400 italic font-medium px-1">Ghi chú: Đổi trạng thái ngày để thay đổi cách tính giờ OT (sau ca hoặc cả ngày).</p>
                            </div>
                        )}

                        <div className={`p-6 rounded-[2rem] border flex items-center justify-between ${isSelectedDayWorking ? 'bg-indigo-50/30 border-indigo-100 text-indigo-700' : 'bg-rose-50/30 border-rose-100 text-rose-700'}`}>
                            <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${isSelectedDayWorking ? 'bg-indigo-100 text-indigo-600' : 'bg-rose-100 text-rose-600'}`}>
                                    <CalendarIcon className="w-6 h-6" />
                                </div>
                                <div>
                                    <p className="text-[11px] font-black uppercase tracking-[0.1em]">{isSelectedDayWorking ? 'Ngày đi làm bình thường' : 'Ngày nghỉ / Ngày lễ'}</p>
                                    <p className="text-[10px] font-medium opacity-60 italic">{isSelectedDayWorking ? 'OT tính trên thời gian làm thêm sau ca' : 'OT tính trên toàn bộ thời gian làm'}</p>
                                </div>
                            </div>
                            <div className={`px-4 py-2 rounded-xl text-[10px] font-black border ${isSelectedDayWorking ? 'bg-white border-indigo-200' : 'bg-white border-rose-200'}`}>
                            ...
                            <div className="bg-white p-6 md:p-8 rounded-[2rem] border border-slate-100 shadow-xl space-y-6">
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center px-1">
                                        <label className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Nội dung công việc</label>
                                        <div className={`text-[10px] font-black px-3 py-1.5 rounded-xl border shadow-sm flex items-center gap-2 ${isTet ? 'bg-red-50 text-red-600 border-red-100' : 'bg-indigo-50 text-indigo-700 border-indigo-100'}`}>
                                            <Timer className="w-3.5 h-3.5" />
                                            {calculatedHours.toFixed(1)}h thực lĩnh
                                        </div>
                                    </div>
                                    <textarea value={reason} onChange={(e) => setReason(e.target.value)} className="w-full p-5 bg-slate-50 border border-slate-200 rounded-2xl text-sm md:text-base h-24 resize-none outline-none font-medium text-slate-900 focus:bg-white focus:border-indigo-500 transition-all shadow-inner" placeholder="Mô tả công việc thực hiện..."/>
                                </div>
                                ...
                                {calculatedHours <= 0 && startTime && endTime && startTime >= endTime && (
                                    <p className="text-[9px] font-bold text-rose-500 uppercase text-center mt-2 italic">Giờ kết thúc phải lớn hơn giờ bắt đầu</p>
                                )}
                                {calculatedHours <= 0 && startTime && endTime && startTime < endTime && (
                                    <p className="text-[9px] font-bold text-rose-500 uppercase text-center mt-2 italic">Sau khi trừ giờ nghỉ ({calculateHours(startTime, endTime, selectedDateStr) === 0 ? '0.5h-1h' : ''}), tổng giờ làm phải &gt; 0</p>
                                )}
                            </div>
                        </form>
                    </div>
