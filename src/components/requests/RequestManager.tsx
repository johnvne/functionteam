...
export const RequestManager: React.FC<RequestManagerProps> = ({ user }) => {
  ...
  const fetchRequests = async (silent = false) => {
    ...
    try {
      console.log("Fetching borrowing requests...");
      ...
      if (data) {
        console.log(`Fetched ${data.length} borrowing requests.`);
  ...
  const filteredRequests = useMemo(() => {
    ...
    return requests.filter(req => {
      ...
      const matchesDate = !filterDate || isSameDay(req.borrow_date, filterDate);
    ...
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto pb-24 px-2 md:px-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-4 flex items-center gap-3 bg-white p-3 border border-gray-100 rounded-[1.5rem] shadow-sm">
           ...
           />
           {filterDate && (
             <button onClick={() => setFilterDate('')} className="px-3 py-1.5 bg-gray-100 text-gray-600 text-[10px] font-black rounded-xl hover:bg-gray-200 transition-colors">TẤT CẢ</button>
           )}
           {filterDate !== todayStr && (
      ...
      {filteredRequests.length > 0 && (
        <Card className="p-6 md:p-10 rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl border-none bg-white flex flex-col sm:flex-row items-center justify-between gap-8 px-12">
          <div className="text-[11px] md:text-sm text-gray-500 font-black uppercase tracking-[0.2em] flex items-center gap-3">
             <div className={`w-2 h-2 rounded-full ${isTet ? 'bg-red-600' : 'bg-indigo-600'}`}></div>
             ...
             <span className={`tabular-nums font-black ml-2 ${isTet ? 'text-red-700' : 'text-indigo-705'}`}>{filteredRequests.length} đơn mượn</span>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap justify-center">
            <button 
              ...
              className="p-2.5 rounded-xl border border-gray-100 bg-white text-gray-800 disabled:opacity-20 hover:bg-indigo-50 transition-all active:scale-90 shadow-sm"
            >
                ...
                .map((p, idx, arr) => (
                  <React.Fragment key={p}>
                    ...
                    <button 
                      ...
                      className={`w-9 h-9 md:w-11 md:h-11 rounded-xl text-[10px] font-black transition-all border-2 ${
                        currentPage === p 
                          ? (isTet ? 'bg-red-700 text-white border-red-700 shadow-lg' : 'bg-indigo-600 text-white border-indigo-600 shadow-lg') 
